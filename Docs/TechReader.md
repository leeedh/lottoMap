...
* **ETL Crawler** (예정)
  ...
  * 로또6/45(LOTTO) 및 연금복권 720+(PENSION) 모두 적재
    * 연금복권은 `등수`에 `보너스`가 포함될 수 있으므로, 원천 값(예: '1등','2등','보너스')을 정규화하여 저장
