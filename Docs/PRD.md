...
2. 좌측 사이드바에서 복권 종류 필터 선택 (**로또 6/45**, **연금복권 720+**) → 지도에서 해당 종류의 판매점이 마커로 표시됨.
...
| **판매점 필터링**    | 좌측 사이드바에서 복권 종류 필터 (**로또 6/45**, **연금복권 720+**) | ✅ 구현됨 | 상    |
...
* **데이터 소스**: 목업 데이터 (`constants.ts`의 `MOCK_STORES`)
  * 초기 적재(백필) 데이터:
    * `lotto-crawling/lotto_all_rounds.csv` (로또6/45 당첨 판매점)
    * `lotto-crawling/pension_all_rounds.csv` (연금복권 720+ 당첨 판매점)
