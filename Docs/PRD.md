...
2. 좌측 사이드바에서 복권 종류 필터 선택 (**로또 6/45**; 연금복권은 향후 데이터 연동 후 지원) → 지도에서 해당 종류의 판매점이 마커로 표시됨.
...
| **판매점 필터링**    | 좌측 사이드바에서 복권 종류 필터 (**로또 6/45**). (연금복권은 향후 데이터 연동 후 지원) | ✅ 구현됨 | 상    |
...
* **데이터 소스**: 목업 데이터 (`constants.ts`의 `MOCK_STORES`)
  * 초기 적재(백필) 데이터: `lotto-crawling/lotto_all_rounds.csv` (**로또6/45 당첨 판매점만**)
* **데이터 구조**: TypeScript 인터페이스로 정의 (`types.ts`)
  * `Store`: 판매점 정보 (이름, 주소, 좌표, 당첨 통계, 이력)
  * `WinRecord`: 당첨 기록 (회차, 등수, 방법, 날짜)
  * `WinStats`: 당첨 통계 (로또 1등, 로또 2등)  ※ 연금복권은 향후 데이터 연동 시 확장
...
1. **Crawler**
   ...
   * (권장) 동행복권 내부 **JSON API**로 회차별/등수별 당첨 판매점 데이터를 수집(가능하면 API 1순위)
     * 네트워크/차단 정책 등으로 직접 호출이 불안정할 수 있으므로, **Playwright 기반 HTML 추출(현재 lotto-crawling 구현)**을 백업 경로로 유지
   * 수집 → 정규화(주소/방법/좌표) → DB 적재(또는 Pub/Sub 메시지 발행)
...
4. **Idempotency(중복 방지)**
   ...
   * 해시 입력에는 최소 `round_no`, `lottery_type`, `source_store_id`, `rank`, `source_seq(번호)`를 포함(동일 회차/판매점 다중 당첨까지 안정적으로 중복 방지)
