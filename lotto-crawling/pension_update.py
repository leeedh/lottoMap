"""
연금복권720+ 당첨 판매점 수동 갱신 스크립트

최신 회차 데이터를 수집하여 기존 CSV에 추가합니다.

사용법:
    python pension_update.py                    # 최신 회차 수집
    python pension_update.py --csv output.csv   # 다른 CSV 파일에 저장
"""
...
class PensionUpdater:
    ...
    async def get_site_latest_round(self) -> int:
        ...
        try:
            ...
            # 셀렉터가 로드될 때까지 대기
            await self.page.wait_for_selector('select#srchLtEpsd', state='visible', timeout=15000)
    ...
    async def update_latest(self) -> bool:
        """최신 회차 확인 및 업데이트"""
        print("\n" + "="*50)
        print(f"🔍 연금복권720+ 최신 회차 확인 중... ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})")
        print("="*50)
...
async def main():
    parser = argparse.ArgumentParser(description='연금복권720+ 당첨 판매점 수동 갱신')
    parser.add_argument('--csv', type=str, default=DEFAULT_CSV_FILE,
                        help=f'CSV 파일 경로 (기본값: {DEFAULT_CSV_FILE})')

    args = parser.parse_args()

    updater = PensionUpdater(csv_file=args.csv)
    await updater.update_latest()
