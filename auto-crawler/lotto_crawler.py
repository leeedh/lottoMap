...
class LottoStoreCrawler:
    ...
    async def select_lottery_type(self, lottery_type: str):
        """
        복권 종류 선택

        Args:
            lottery_type: 복권 종류 ("로또6/45", "연금복권720+", "스피또2000", "스피또1000", "스피또500")
        """
        lottery_map = {
            "로또6/45": "lt645",
            "연금복권720+": "pt720",
        ...
        if lottery_type in lottery_map:
            value = lottery_map[lottery_type]
            # select 박스에서 선택
            await self.page.select_option('select#ltGds', value)
            await asyncio.sleep(2)  # 데이터 로드 대기
            print(f"✅ 복권 종류 선택: {lottery_type}")
        else:
            print(f"⚠️  지원하지 않는 복권 종류: {lottery_type}")
    ...
    async def get_all_regions_stores(self) -> List[Dict]:
        """
        모든 지역의 판매점 정보 수집
        ...
        # 지역 버튼 가져오기
        region_buttons = await self.page.query_selector_all('.srchShpLctn')
        
        print(f"📍 총 {len(region_buttons)}개 지역 탐색 시작")
        
        for i, button in enumerate(region_buttons):
            try:
                # 지역명 가져오기
                region_name = await button.text_content()
                print(f"\n[{i+1}/{len(region_buttons)}] {region_name} 크롤링 중...")
                ...
                print(f"⚠️  지역 크롤링 중 오류: {e}")
                continue
        
        print(f"\n🎉 전체 {len(all_stores)}개 판매점 정보 수집 완료!")
        return all_stores
    ...
    def save_to_csv(self, stores: List[Dict], filename: str = None):
        ...
        if filename is None:
            ...
            filename = f"lotto_stores_{timestamp}.csv"
    ...
    async def crawl_round_fast(self, round_num: str, verbose: bool = False) -> List[Dict]:
        """
        특정 회차 빠른 크롤링 (최소 대기시간)
        ...
        try:
            ...
            stores = await self.get_stores_silent()
            ...
            if verbose:
                print(f"  ✅ {round_num}회: {len(stores)}개 판매점")

            return stores
        except Exception as e:
            if verbose:
                print(f"  ⚠️ {round_num}회 크롤링 실패: {e}")
            return []
...
class ParallelLottoCrawler:
    ...
    async def _worker(self, worker_id: int, rounds: List[str], results: List,
                      semaphore: asyncio.Semaphore, progress: dict):
        """
        워커 함수 - 할당된 회차들을 크롤링
            ...
            try:
                ...
                for round_num in rounds:
                    try:
                        ...
                        await page.wait_for_selector('.store-box', state='visible', timeout=10000)
                        ...
                        progress['stores'] += len(stores)

                    except Exception as e:
                        progress['failed'] += 1

            except Exception as e:
                print(f"  ⚠️ 워커 {worker_id} 초기화 실패: {e}")
            ...
            for i, round_num in enumerate(all_rounds):
                try:
                    ...
                    if save_interval and (i + 1) % save_interval == 0:
                        temp_filename = f"lotto_checkpoint_{i+1}.csv"
    ...
    def save_to_csv(self, stores: List[Dict], filename: str = None):
        ...
        if filename is None:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = f"lotto_all_rounds_{timestamp}.csv"
...
async def crawl_all_rounds_example():
    """전체 회차 크롤링 예제"""
    # 병렬 크롤러 생성 (워커 3개)
    ...
    # CSV 저장
    crawler.save_to_csv(all_stores, "lotto_all_rounds.csv")
...
async def main():
    ...
    try:
        ...
        # 옵션 1: 기본 설정으로 현재 페이지 크롤링
        print("\n📊 [옵션 1] 기본 설정으로 크롤링")
        stores = await crawler.get_stores()
        crawler.save_to_csv(stores, "lotto_stores_default.csv")

        # 옵션 2: 특정 회차 선택하여 크롤링
        print("\n📊 [옵션 2] 특정 회차 (1206회) 크롤링")
        await crawler.select_lottery_type("로또6/45")
        await crawler.select_round("1206")
        await crawler.select_rank("전체")
        stores = await crawler.get_stores()
        crawler.save_to_csv(stores, "lotto_stores_1206.csv")

        # 옵션 3: 모든 지역 크롤링 (시간이 오래 걸릴 수 있음)
        print("\n📊 [옵션 3] 모든 지역 크롤링")
